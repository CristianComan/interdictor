from interdictor.mode_history import ModeHistory
...
def test_record_returns_and_stores_transition():
    history = ModeHistory()
    transition = history.record("STANDBY", "JAM_GNSS_L1", "task-1")
...
def test_records_accumulate_in_order():
    history = ModeHistory()
    history.record("STANDBY", "JAM_GNSS_L1", "task-1")
    history.record("JAM_GNSS_L1", "STANDBY", "task-2")

    modes = [(t.from_mode, t.to_mode) for t in history.transitions]
    assert modes == [("STANDBY", "JAM_GNSS_L1"), ("JAM_GNSS_L1", "STANDBY")]
    assert history.last.to_mode == "STANDBY"
